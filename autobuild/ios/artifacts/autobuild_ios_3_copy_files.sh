...
echo ""
echo ""
artifact_deploy_filename=koord-jamulus_${jamulus_buildversionstring}_ios${BUILD_SUFFIX}.app
echo "Move/Rename the built file to deploy/${artifact_deploy_filename}"
mv "${THIS_JAMULUS_PROJECT_PATH}"/deploy/Koord-Jamulus*.app "${THIS_JAMULUS_PROJECT_PATH}"/deploy/"${artifact_deploy_filename}"
