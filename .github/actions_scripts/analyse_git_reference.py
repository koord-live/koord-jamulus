...
# It analyzes Koord-RT.pro and git push details (tag vs. branch, etc.) to decide
...
def get_version_from_jamulus_pro():
    with open(REPO_PATH + '/Koord-RT.pro', 'r') as f:
        pro_content = f.read()
    m = re.search(r'^VERSION\s*=\s*(\S+)$', pro_content, re.MULTILINE)
    if not m:
        raise Exception("Unable to determine Koord-RT.pro VERSION")
    return m.group(1)
...
if publish_to_release:
    ...
    if not is_prerelease and build_version != release_tag[1:].replace('_', '.'):
        raise Exception(f"non-pre-release tag {release_tag} doesn't match Koord-RT.pro VERSION = {build_version}")
