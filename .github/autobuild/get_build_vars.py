...
# It analyzes Koord.pro and git push details (tag vs. branch, etc.) to decide
...
def get_version_from_jamulus_pro():
    ...
    if not m:
        raise Exception("Unable to determine Koord.pro VERSION")
    return m.group(1)
...
if publish_to_release:
    ...
    if not is_prerelease and build_version != release_tag[1:].replace('_', '.'):
        raise Exception(f"non-pre-release tag {release_tag} doesn't match Koord.pro VERSION = {build_version}")
